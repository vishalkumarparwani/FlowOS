...
router = APIRouter(prefix="/backlog", tags=["backlogs"])
...
def update_task(backlog_id: int, task: BacklogCreate, db: Session = Depends(get_db)):
...
def delete_task(backlog_id: int, db: Session = Depends(get_db)):
    db_backlog = db.query(Backlog).filter(Backlog.id == backlog_id).first()
